...
for loopIterator in loopRange:
	...
	if MissingValue not in [kphit, rt_a1, tvdss] :
		...
		if FWL-tvdss > 0:
			if rt_a1 == 1:
				min_sw=0.49
				swl = -0.0009*kphit+0.6339
				swat = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.081*y_cos_theta))**(1/-3.304)
				swat_final_new = 1 - (1-swl)/(1-min_sw)*(1-swat)
				swat_final = swat * (1-min_sw) + min_sw
				sowcr = 1 - min_sw - (0.075*log(kphit)+0.2566)*(1-min_sw)
			elif rt_a1 == 2:
				min_sw=0.44
				swl = -0.0009*kphit+0.6339
				swat = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.0656*y_cos_theta))**(1/-4.835)
				swat_final_new = 1 - (1-swl)/(1-min_sw)*(1-swat)
				swat_final = swat * (1-min_sw) + min_sw
				sowcr = 1 - min_sw - (0.075*log(kphit)+0.2566)*(1-min_sw)
			elif rt_a1 == 3:
				min_sw=0.65
				swl = -0.0009*kphit+0.6339
				swat = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.1173*y_cos_theta))**(1/-3.251)
				swat_final_new = 1 - (1-swl)/(1-min_sw)*(1-swat)
				swat_final = swat * (1-min_sw) + min_sw
				sowcr = 1 - min_sw - (0.075*log(kphit)+0.2566)*(1-min_sw)
