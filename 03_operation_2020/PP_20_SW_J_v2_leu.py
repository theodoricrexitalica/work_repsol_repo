...
for loopIterator in loopRange:
	...
	if MissingValue not in [kphit, rt_a1, tvdss, swir] :
		...
		if FWL-tvdss > 0:
			if rt_a1 == 1:
				#swnj=10
				swnj = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.1253*y_cos_theta))**(1/-2.616)
			elif rt_a1 == 2:
				#swnj=20
				swnj = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.0888*y_cos_theta))**(1/-4.039)
			elif rt_a1 == 3:
				#swnj=30
				swnj = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.1347*y_cos_theta))**(1/-2.307)
