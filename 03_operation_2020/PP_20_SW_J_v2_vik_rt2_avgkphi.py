...
#Description:Description
#Format:auto
HAFWL = Variable("11-R", "Slb_WLL", "HAFWL", u"Height Above Free Water Level", u"m")
parameterDict.update({'HAFWL' : Parameter(name='HAFWL',bname='H',type='Variable',family='Height Above Free Water Level',measurement='',unit='m',value='11-R.Slb_WLL.HAFWL',mode='Out',description='Description',group='',min='',max='',list='',enable='True',iscombocheckbox='False',isused='True')})
#Type:Variable
#BName:swj
...
#Description:Description
#Format:auto
SW_J = Variable("11-R", "Slb_WLL", "SW_J", u"Water Saturation", u"v/v")
parameterDict.update({'SW_J' : Parameter(name='SW_J',bname='swj',type='Variable',family='Water Saturation',measurement='',unit='v/v',value='11-R.Slb_WLL.SW_J',mode='Out',description='Description',group='',min='',max='',list='',enable='True',iscombocheckbox='False',isused='True')})
...
for loopIterator in loopRange:
	...
	H = MissingValue
	swj = MissingValue
	if MissingValue not in [kphit, tvdss] :
		### Automatic Generation Loop End ###
		if FWL-tvdss > 0:
			swj = ((3.183*(FWL-tvdss)*(rho_w-rho_o)*0.098*sqrt(kphit))/(0.126*y_cos_theta))**(1/-4.628)
